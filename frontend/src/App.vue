<template>
  <div class="flex flex-col min-h-screen">
    <header class="w-full bg-gray-900 text-white py-6">
      <div
        class="max-w-7xl mx-auto flex items-center justify-between px-4 sm:px-6 xl:px-8"
      >
        <div class="flex items-center space-x-4">
          ...
          <nav class="hidden sm:flex space-x-4">
            <router-link to="/" class="text-lg font-medium hover:text-blue-400"
              >Главная</router-link
            >
            <router-link
              to="/categories"
              class="text-lg font-medium hover:text-blue-400"
        ...
        <div class="sm:hidden">
          ...
          <div
            v-if="menuOpen"
            class="absolute top-16 right-0 mt-2 w-48 rounded-md shadow-lg py-1 bg-white ring-1 ring-black ring-opacity-5"
          >
            <router-link
              to="/"
            ...
            <button
              v-if="user.isAuthenticated"
              @click="userSignOut"
              class="block w-full text-left px-4 py-2 text-lg text-gray-700 hover:bg-gray-100"
            >
              Выйти
            </button>
          </div>
